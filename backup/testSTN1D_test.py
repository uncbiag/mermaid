...
start = time.time()
my_lib_nd.BilinearSamplerBCXYZ_updateOutput_ND(inputImage_cuda, inputGrids_cuda, output_cuda, 1)
...
start = time.time()
my_lib_nd.BilinearSamplerBCXYZ_updateGradInput_ND(inputImage, inputGrids,grad_input , grad_grids, grad_output,1, False)

print('cpu backward time taking:', time.time() - start)

#grad_output_cuda=grad_output.cuda(device)
start = time.time()
my_lib_nd.BilinearSamplerBCXYZ_updateGradInput_ND(inputImage_cuda, inputGrids_cuda, grad_input_cuda, grad_grids_cuda, grad_output_cuda,
                                                  1, True)
