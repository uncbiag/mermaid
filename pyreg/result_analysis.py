...
class XlsxRecorder(object):
    ...
    def init_summary(self):
        ...
        writer.save()
        writer.close()
        self.xlsx_writer = pd.ExcelWriter(os.path.join(self.saving_path, 'results.xlsx'), engine='openpyxl')
    ...
    def put_into_avg_buff(self, result, iter_info):
        """
        # summed_buffer is to save summed_results from each iter, from each batch
        ...
        """
        if iter_info not in self.summed_buffer:
            self.summed_buffer[iter_info] = []
            self.iter_info_buffer += [iter_info]
        self.summed_buffer[iter_info] += [result]
    ...
    def merge_from_avg_buff(self):
        ...
        """
        metric_summed_dic={}
        for iter_info,summed_list in self.summed_buffer.items():
            metric_results_tmp = {metric: [np.squeeze(result[metric]) for result in summed_list] for metric in
                                  self.measures}
            metric_summed_dic[iter_info] = {metric: np.concatenate(metric_results_tmp[metric], 0) for metric in metric_results_tmp}
        return  metric_summed_dic
    ...
    def saving_results(self,sched, results=None, info=None, averaged_results=None):
        ...
        if sched == 'batch':
            ...
            metric_summed_dic = self.merge_from_avg_buff()
            self.saving_label_averaged_results(metric_summed_dic)
            self.saving_summary(metric_summed_dic)
    ...
    def saving_label_averaged_results(self, results):
        ...
        # saving the n_File*nSummedMetrics into xlsx_writer
        # including the iter_info
        start_column = 0
        results_summary = {iter_info: {metric:np.sum(results[iter_info][metric])} for metric in self.measures for iter_info in self.iter_info_buffer}
    ...
    def saving_summary(self, results):
        # saved by iteration
        # saving the 1*nSummedMetrics into summary_book_path
        ...
        results_summary = {metric+iter_info: np.sum(results[iter_info][metric]) for metric in self.measures for iter_info in self.iter_info_buffer}
        ...
        df.to_excel(self.summary_writer, startrow = startrow, index_label=self.task_name)
