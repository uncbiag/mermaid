...
import numpy as np
from dataWapper import USE_CUDA, MyTensor, AdpatVal
...
class Smoother(object):
    ...
    def smooth_scalar_field_multiNC(self, I, Iout=None):
        ...
        if Iout is not None:
            ...
            Is = AdpatVal(Variable(MyTensor(sz).zero_(), requires_grad=False))
    ...
    def inverse_smooth_vector_field_multiN(self, v, vout=None):
        ...
        if vout is not None:
            ...
            ISv = AdpatVal(Variable(MyTensor(v.size()).zero_()))
    ...
    def smooth_vector_field_multiN(self, v, vout=None):
        ...
        if vout is not None:
            ...
            Sv = AdpatVal(Variable(MyTensor(v.size()).zero_()))
...
class GaussianSpatialSmoother(GaussianSmoother):
    ...
    def _create_filter(self):
        ...
        ###################################
        if self.dim==1:
            self.filter =AdpatVal(Variable(torch.from_numpy(self.smoothingKernel)))
        elif self.dim==2:
            self.filter = AdpatVal(Variable(torch.from_numpy(self.smoothingKernel)))
        elif self.dim==3:
            self.filter = AdpatVal(Variable(torch.from_numpy(self.smoothingKernel)))
